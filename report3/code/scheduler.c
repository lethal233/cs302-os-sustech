...
/*if the priority is higher, then choosing it from the READY queue, or the priority is the same, then choosing the longest wait_time modified*/
...
void jobswitch()
{
	...
	if (next == NULL && current == NULL)          /* no job to run */
		...
		current->job->curpri = current->job->defpri;// to set the pri to the initial pri
...
void do_deq(struct jobcmd deqcmd)
{
	...
	if (current && current->job->jid == deqid) {
		...
		kill(SIGTERM, current->job->pid);// to terminate it
...
void do_stat()
{
	...
	*/
	...
	if (current) {
		...
		printf("%d\t%d\t%s\t%d\t%d\t%d\t\t%d\t%d\t%d\t%s\n",
			current->job->jid, //int
			current->job->pid, //int 
			current->job->cmdarg[0],//string
			current->job->ownerid,//int 
			current->job->run_time,// int 
			current->job->wait_time,//int 
			current->job->defpri,//int 
			current->job->curpri,//int 
			current->job->state,//int
			timebuf,//string
			"RUNNING" );
	}
